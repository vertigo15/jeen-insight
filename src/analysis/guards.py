...
from src.analysis.contracts import GuardExit, GuardResult, field_bounds
...
def _window_bounds(skill: Optional[str]) -> Optional[tuple]:
    """The skill's own ``window`` bounds (seasonality's floor is 24, not 12)."""
    if not skill:
        return None
    try:
        lo, hi = field_bounds(skill, "window")
    except KeyError:
        return None
    if lo is None and hi is None:
        return None
    return (lo if lo is not None else 12, hi if hi is not None else 1500)


def _coarser_exit(sf: SeriesFrame, *, recommended: bool, horizon: Optional[int] = None,
                  window: Optional[int] = None, skill: Optional[str] = None) -> Optional[GuardExit]:
    ...
    return GuardExit(
        ...
        params_patch=grain_patch(sf.grain, coarser, horizon=horizon, window=window, window_bounds=_window_bounds(skill)),
...
def _finer_exit(sf: SeriesFrame, *, recommended: bool, horizon: Optional[int] = None,
    ...
    return GuardExit(
        ...
        params_patch=grain_patch(sf.grain, finer, horizon=horizon, window=window, window_bounds=_window_bounds(skill)),
        recommended=recommended,
    )
...
def series_length(sf: SeriesFrame, *, horizon: Optional[int] = None, window: Optional[int] = None,
                  skill: Optional[str] = None) -> GuardResult:
    n = sf.n
    ok = n >= SERIES_LENGTH_MIN
    exits: List[GuardExit] = []
    if not ok:
        exits.append(_wider_window_exit(sf, max(SERIES_LENGTH_MIN * 2, 24)))
        finer = _finer_exit(sf, recommended=True, horizon=horizon, window=window, skill=skill)
        if finer:
            exits.append(finer)
...
def gap_ratio(sf: SeriesFrame, *, horizon: Optional[int] = None, window: Optional[int] = None,
              skill: Optional[str] = None) -> GuardResult:
    ratio = sf.gap_ratio
    ok = ratio <= GAP_RATIO_MAX
    exits: List[GuardExit] = []
    if not ok:
        coarser = _coarser_exit(sf, recommended=True, horizon=horizon, window=window, skill=skill)
        if coarser:
...
def min_history(sf: SeriesFrame, **_: object) -> GuardResult:  # accepts horizon/window/skill like its peers
...
def intermittent(sf: SeriesFrame, *, horizon: Optional[int] = None, window: Optional[int] = None,
                 informational: bool = False, skill: Optional[str] = None) -> GuardResult:
    ...
    that engine adds Croston/ADIDA/IMAPA to its ``auto`` shortlist instead of
    refusing, and a refusal whose exit changed the method would only refuse
    again. The engine's own note says what it did with the zeros."""
    share = sf.zero_share
    ...
    if over and informational:
        detail += "; the forecast engine handles intermittent demand itself (see its notes)"
    return GuardResult(
        name="intermittent",
        passed=ok,
        detail=detail,
    ...
    *,
    ...
    for name in guard_names:
        ...
        if name == "intermittent":
            results.append(intermittent(sf, horizon=horizon, window=window, skill=skill,
                                        informational=skill in INTERMITTENT_INFORMATIONAL_SKILLS))
            continue
        fn = _GUARDS.get(name)
        if fn is None:
            continue
        results.append(fn(sf, horizon=horizon, window=window, skill=skill))
    return results
