...
def _forecast_rules(analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # 2. No seasonal term because the window could not hold a full cycle, and
    ...
    if period and n is not None and not facts.get("seasonal_periods") and _window_was_the_limit(facts, params):
        if grain == "day":
            # The weekly season (7) is always testable at 90+ days; what is
            # missing is the yearly one — advise the weekly grain instead.
            if int(n) < _YEARLY_DAILY_POINTS and coarser:
                out.append(_adjustment(
                    "coarser_grain_for_season", grain_patch(grain, coarser, horizon=horizon, window=window),
                    grain=coarser,
                ))
        else:
