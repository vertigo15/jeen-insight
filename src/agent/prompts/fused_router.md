...
- **capability** — The question is about THIS assistant itself: what it can do, which analyses or ML models exist and how to trigger them, or whether/how the model can be changed. Examples: "what does this app do?", "what can you do?", "which ML models can I use?", "how do I run a forecast?", "can I change the anomaly method to 3-sigma?". Note: a request that actually asks for data or a number is `needs_query`/`needs_analysis`, NOT `capability`.
...
```json
{{"route": "<needs_query|needs_analysis|from_memory|history_lookup|capability|out_of_scope|unsafe>", "reason": "<one sentence>", "confidence": <0.0-1.0>, "prior_refs": ["T3"], "history_query": {{"keywords": ["revenue"], "since": "2026-09-13", "until": null}}}}
```
