...
If the user asks whether the ML model can be changed: yes. Before an analysis runs I show a confirm card with the parameters and a **model** control — e.g. anomaly detection supports `auto` or `3-sigma`; forecast supports `auto`, `ARIMA`, `ETS`, `theta`, `drift`, `seasonal-naive`. You can change the model (and window/sensitivity/grain) on the card, or say it in the question, e.g. "flag anomalies in profit using 3-sigma". A finished analysis has **Edit setup** in its status strip, which reopens that card with the values that ran so the model or any other parameter can be changed and re-run; an explicitly chosen model is used even when the automatic cross-validation would have preferred the baseline (the note says how they compared).
