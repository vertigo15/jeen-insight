    ...
    function accuracyHtml(analysis, tracking) {
        ...
        if (state.error) {
            ...
            if (!ev.elapsed_periods) {
                ...
                const stats = [
                    kv(t('analysis.accuracy.realizedError', { metric: r.metric || '' }), `${fmtMetric(r.metric, r.value)}${r.band && r.band !== 'n/a' ? ` · ${esc(r.band)}` : ''}`),
                    kv(t('analysis.accuracy.claimedError', { metric: c.metric || '' }), `${fmtMetric(c.metric, c.value)}${c.band && c.band !== 'n/a' ? ` · ${esc(c.band)}` : ''}`),
                ...
                body = `<div class="v3-stats">${stats}</div>
                  <div class="v3-ml-accuracy-note">${h('analysis.accuracy.scored', { elapsed: ev.elapsed_periods, pending: ev.pending_periods, dataEnd: ev.data_end })}</div>
