...
@pytest.mark.asyncio
async def test_postgres_store_passes_typed_arguments_to_asyncpg():
    """asyncpg's codecs want datetime.date / UUID / Decimal, not strings — the
    in-memory double cannot see a regression here, so a fake connection records
    what the Postgres store would have sent."""
    ...
    class _Conn:
        ...
        async def executemany(self, sql, rows):
            calls.append(("executemany", sql, rows))
    ...
    env = _forecast_env(n=60, horizon=4).model_dump(mode="json")
    store = AnalysisStore(_Pool(), schema_ready=True, forecast_schema_ready=True)
    qid = "11111111-1111-1111-1111-111111111111"
    ok = await store.record_forecast(query_id=qid, user_id="u", source_key="db", session_id="22222222-2222-2222-2222-222222222222",
                                     params={"series": series_request(), "window": 60, "horizon": 4}, envelope=env)
    assert ok is True
    insert = next(c for c in calls if c[0] == "execute" and "INSERT INTO insights_forecast_runs" in c[1])
    args = insert[2]
    assert isinstance(args[0], UUID) and isinstance(args[3], UUID)          # query_id, session_id
    assert isinstance(args[9], Decimal)                                     # interval_level → NUMERIC(4,3)
    assert isinstance(args[16], date) and not isinstance(args[16], type(None))  # history_end
    assert "evaluation = NULL" in insert[1]                                 # a re-capture resets the old evaluation
    points = next(c for c in calls if c[0] == "executemany")
    assert len(points[2]) == 4
    assert all(isinstance(r[0], UUID) and isinstance(r[2], date) and isinstance(r[3], float) for r in points[2])
    # Never a cast that would fight the typed value.
    assert "::date" not in insert[1] and "::date" not in points[1]
