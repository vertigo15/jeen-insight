...
TIER_ORDER = ["exact", "equivalent", "structural", "execution_match", "mismatch"]
...
def structural_equal(a: Dict[str, Any], b: Dict[str, Any]) -> bool:
    return (
        ...
        and a["filter_literals"] == b["filter_literals"]
        and a["limit"] == b["limit"]
    )
...
def fingerprint_delta(gold: Dict[str, Any], gen: Dict[str, Any]) -> Dict[str, Any]:
    ...
    delta: Dict[str, Any] = {}
    for key in ("tables", "aggregates", "group_by", "filter_literals"):
    ...
    if gold["limit"] != gen["limit"]:
        delta["limit"] = {"gold": gold["limit"], "generated": gen["limit"]}
    return delta
...
def _tagged(fp: Dict[str, Any]) -> Set[str]:
    ...
    items.update(f"filter:{f}" for f in fp["filter_literals"])
...
def compare_execution(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if payload.get("app_truncated") and len(app_keys) < len(gold_keys):
        ...
        missing = [k for k in app_keys if k not in gold_set]
        return {
            "match": not missing,
            "detail": "app rows are a subset of gold rows (app result truncated)" if not missing else f"{len(missing)} app row(s) not in gold",
